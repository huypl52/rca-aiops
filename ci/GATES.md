...
| 6 | Benchmark determinism (11-scenario inject + full-agent conjunction + graded scoring + playbook-retrieval SM-4 + train/eval split Q8) | FR-10, AD-13 #6 | **DETERMINISM WIRED** ✓ (inject + conjunction + scoring + SM-4/Q8 done) | Story 6.1 + 6.2 + 6.3 + 6.4 | `tests/ci/test_gate6_*determinism.py` |
...
- **Bind:** `uv run pytest tests/ci/test_gate6_benchmark_determinism.py tests/ci/test_gate6_conjunction_determinism.py tests/ci/test_gate6_scoring_determinism.py tests/ci/test_gate6_retrieval_determinism.py -v`.
...
- **(d) Playbook-retrieval (SM-4 / FR-10e / A6) + train/eval split (Q8 / FR-10) determinism (Story 6.4):**
  TWO new INDEPENDENT columns ABOVE the 6.1/6.2/6.3 baselines (R1 — those harnesses are byte-for-byte
  unchanged): SM-4 = the playbook-retrieval independent axis (a SEPARATE column — never folded into SM-1
  correctness, R2) reading the REAL `playbook_hits` from the terminal-state spine; Q8 = the DETERMINISTIC
  train/eval partition (index-based over the FROZEN `BENCHMARK_SCENARIOS` tuple, leak-free). Computed over
  all 11 → a byte-identical retrieval blob across `PYTHONHASHSEED={0,1,42}` (§2C — spawns
  `python -m tests.retrieval_harness`). SM-4 = set-membership counts + a `float` recall; Q8 = SORTED
  partition lists + order-independent leak-free `isdisjoint`/union checks — all hash-seed-independent;
  canonical JSON. Byte-stability proves the WHOLE SM-4 + Q8 computation is hash-seed-stable.
- **Boundary (6.4 extends):** gate #6 asserts the inject + conjunction + graded-scoring + SM-4/Q8 axes RUN
  + are DETERMINISTIC. It does NOT assert `SM-1 ≥ threshold`, `coverage ≥ threshold`, `SM-3 ≥ threshold`,
  NOR a tolerance PASS, NOR `SM-4 recall ≥ threshold`, NOR a retrieval PASS, NOR a specific Q8 split ratio
  (D4 — confidence/SM/tolerance/retrieval/ratio cutoffs defer) — the honest baselines are `SM-1 = 0%` (the
  ...
  `blocked`, SM-4 `EMPTY/DEGRADED` (`playbook_hits=[]` 11/11 — the retriever is wired but the Qdrant corpus
  is absent, 5-A1-family; R1: do NOT add retrieval content), Q8 `leak_free=True` /
  `calibration="moot-no-reports"` (no reports to calibrate D3/D4 on — 5-A1). The retrieval PASS + cutoffs +
  the split ratio land at convergence (R2 — SM-4 is a SEPARATE supplementary axis, never a back-door pass).
